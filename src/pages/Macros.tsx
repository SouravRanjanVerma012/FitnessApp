...
const ResultsDisplay: React.FC<{ results: MacroResults }> = ({ results }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Save to backend
      const response = await fetch(`${apiService.baseURL}/workouts/macro-summary`, {
        method: 'POST',
        headers: {
...
=====================================`;
  ...
  const handleSaveWorkoutProgress = async () => {
    try {
      ...
      const workoutResponse = await fetch(`${apiService.baseURL}/workouts`, {
...
const Macros = () => {
  ...
  const { data: macroSummariesData } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${apiService.baseURL}/workouts/macro-summaries`, {
        headers: { 'Authorization': `Bearer ${apiService.getToken()}` },
        cache: 'no-store'
      });
